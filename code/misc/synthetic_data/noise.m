...
load meanpower  	%number of sinusoids from which each simulated signal is composed of

signal = zeros (frames, epochs);
...
for trial = 1:epochs
   ...
   for i = 1:sumsig
      ...
      phase = rand(1)*2*pi;
      s = signal (range) + sin ([1:frames]/srate*2*pi*freq + phase) * freqamp;
   end
   signal(:,trial) = s;
end